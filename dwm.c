...
struct Client {
    ...
    int isfixed, isfloating, iscentered, canfocus, isurgent, isalwaysontop, neverfocus, oldstate, isfullscreen,
        resizehints, isterminal, noswallow, issticky, isforegrounded;
    int beingmoved;
    int floatborderpx;
    int hasfloatbw;
    pid_t pid;
...
static void previewnext(const Arg *arg);
static void previewprev(const Arg *arg);
static void previewopen(const Arg *arg);
static void previewcancel(const Arg *arg);
...
static XImage *previewblankimage(void);
static void previewhighlight();
static Client *previewclose(Monitor *m, Client *target);
...
static int previewactive = 0;     /* preview overlay is being shown */
static Client *previewsel = NULL; /* currently highlighted client in the preview */
...
static void alttab(const Arg *arg) {
    ...
    for (int i = 0; i < 100; i += 1) {
        ...
        if (grabbed_keyboard == GrabSuccess) {
            XGrabButton(dpy, AnyButton, AnyModifier, None, False, BUTTONMASK, GrabModeAsync, GrabModeAsync, None, None);
            break;
        }
        nanosleep(&ts, NULL);
        if (i == 100 - 1) grabbed = 0;
    }
    ...
    while (grabbed) {
        ...
        switch (event.type) {
        case KeyPress:
            if (event.xkey.keycode == tabCycleKey) focusnext(&(Arg){.i = alt_tab_direction});
            break;
        case KeyRelease:
...
Client *cropwintoclient(Window w) {
    ...
    for (m = mons; m; m = m->next)
        for (c = m->clients; c; c = c->next)
            if (c->crop && c->crop->win == w) return c;
    return NULL;
}
...
void cropwindow(Client *c) {
    ...
    if (!c->crop) {
        ...
        c->win = XCreateWindow(dpy, root, x, y, 1, 1, c->bw, 0, 0, 0, CWEventMask, &wa);
        XReparentWindow(dpy, c->crop->win, c->win, 0, 0);
        XMapWindow(dpy, c->win);
        focus(c);
        XCheckTypedWindowEvent(dpy, c->crop->win, UnmapNotify, &ev);
        if (XCheckTypedWindowEvent(dpy, root, UnmapNotify, &ev) && ev.xunmap.window != c->crop->win)
            XPutBackEvent(dpy, &ev);
    }
    resizeclient(c->crop, c->crop->x + c->x - x, c->crop->y + c->y - y, c->crop->w, c->crop->h);
...
void cropresize(Client *c) {
    resizeclient(c->crop, BETWEEN(c->crop->x, -(c->crop->w), 0) ? c->crop->x : 0,
                 BETWEEN(c->crop->y, -(c->crop->h), 0) ? c->crop->y : 0, c->crop->w, c->crop->h);
    resize(c, c->x, c->y, MIN(c->w, c->crop->x + c->crop->w), MIN(c->h, c->crop->y + c->crop->h), 0);
}
...
void cleanup(void) {
    ...
    for (i = 0; i < LENGTH(colors) * SchemeN; i++) free(schemes[i]);
    free(schemes);
...
void changeopacity(const Arg *arg) {
    ...
    if (selmon->sel->opacity < 0) selmon->sel->opacity = 0;
...
void clientmessage(XEvent *e) {
    ...
    if (!c && !(c = cropwintoclient(cme->window))) return;
...
void detach(Client *c) {
    ...
    for (int i = 1; i < LENGTH(tags); i++)
        if (c == c->mon->tagmarked[i]) c->mon->tagmarked[i] = NULL;
    ...
    if ((w = m->ww - tw - stw - x) > bh) {
        if (m->sel) {
            ...
            if (m->sel->isfloating) {
                ...
                if (m->sel->isalwaysontop) drw_rect(drw, x + boxs, bh - boxw, boxw, boxw, 0, 0);
            }
            if (m->sel->issticky)
...
void drawbars(void) {
    ...
    if (showsystray && !systraypinning) updatesystray(0);
}
...
void enternotify(XEvent *e) {
    ...
    if (!c) c = cropwintoclient(ev->window);
...
void expose(XEvent *e) {
    ...
    if (ev->count == 0 && (m = wintomon(ev->window))) {
        ...
        if (showsystray && m == systraytomon(m)) updatesystray(0);
    }
}
...
void focussame(const Arg *arg) {
    ...
    if (!XGetClassHint(dpy, selmon->sel->win, &ch)) return;
    ...
    for (c = selmon->clients; c && num_clients < 32; c = c->next) {
        if (c->tags & selmon->tagset[selmon->seltags] && XGetClassHint(dpy, c->win, &ch)) {
            if (strcmp(class_name, ch.res_class) == 0) clients[num_clients++] = c;
            XFree(ch.res_class);
            XFree(ch.res_name);
        }
    }
    ...
    if (direction == +1) {
        ...
        if (!target_client) target_client = clients[0];
    } else if (direction == -1) {
        for (int i = 0; i < num_clients; ++i) {
            if (clients[i]->win == lastfocusedwin) {
        ...
        if (!target_client) target_client = clients[num_clients - 1];
    }
...
void focusmaster(const Arg *arg) {
    ...
    if (selmon->sel == master) {
        if (selmon->tagmarked[i] && ISVISIBLE(selmon->tagmarked[i])) focus(selmon->tagmarked[i]);
    } else {
...
void focusmon(const Arg *arg) {
    ...
    selmon = m;
    focus(NULL);
    if (selmon->sel) XWarpPointer(dpy, None, selmon->sel->win, 0, 0, 0, 0, selmon->sel->w / 2, selmon->sel->h / 2);
}
...
void focusnthmon(const Arg *arg) {
    ...
    if ((m = numtomon(arg->i)) == selmon) return;
    unfocus(selmon->sel, 0);
    selmon = m;
    focus(NULL);
}
...
void incnmaster(const Arg *arg) {
    ...
    for (i = 0; i < LENGTH(tags); ++i)
        if (selmon->tagset[selmon->seltags] & 1 << i) selmon->pertag->nmasters[i + 1] = selmon->nmaster;
...
void killclient(const Arg *arg) {
    ...
    if (!selmon->sel || !selmon->sel->allowkill) return;

    if (!arg->ui || arg->ui == 0) {

        if (selmon->sel->crop) cropdelete(selmon->sel);
    ...
    for (c = selmon->clients; c; c = c->next) {
        if (!ISVISIBLE(c) || (arg->ui == 1 && c == selmon->sel)) continue;

        if (arg->ui != 3)
            if (!c->allowkill) continue;

        if (c->crop) cropdelete(c);
...
void keyrelease(XEvent *e) {
    ...
    for (i = 0; i < LENGTH(keys); i++)
        if (momentaryalttags && keys[i].func && keys[i].func == togglealttag && selmon->alttag &&
...
void gesture(const Arg *arg) {
    ...
    if (valid) gestures[listpos].func(&(gestures[listpos].arg));
...
void placemouse(const Arg *arg) {
    ...
    if (!(c = selmon->sel) || !c->mon->lt[c->mon->sellt]->arrange) /* no support for placemouse when
    ...
    do {
        ...
        case MotionNotify:
            ...
            if (!freemove && (abs(nx - ocx) > snap || abs(ny - ocy) > snap)) freemove = 1;

            if (freemove) XMoveWindow(dpy, c->win, nx, ny);

            if ((m = recttomon(ev.xmotion.x, ev.xmotion.y, 1, 1)) && m != selmon) selmon = m;
...
void moveresize(const Arg *arg) {
    ...
    if (!c || !arg) return;
    if (selmon->lt[selmon->sellt]->arrange && !c->isfloating) return;
    if (sscanf((char *) arg->v, "%d%c %d%c %d%c %d%c", &x, &xAbs, &y, &yAbs, &w, &wAbs, &h, &hAbs) != 8) return;

    /* compute new window position; prevent window from be positioned outside the
     * current monitor */
    nw = c->w + w;
    if (wAbs == 'W') nw = w < selmon->mw - 2 * c->bw ? w : selmon->mw - 2 * c->bw;

    nh = c->h + h;
    if (hAbs == 'H') nh = h < selmon->mh - 2 * c->bw ? h : selmon->mh - 2 * c->bw;
    ...
    if (xqp && ox <= msx && (ox + ow) >= msx && oy <= msy && (oy + oh) >= msy) {
        ...
        if ((msx + nmx) > c->x && (msy + nmy) > c->y) XWarpPointer(dpy, None, None, 0, 0, 0, 0, nmx, nmy);
    }
}
...
void moveresizeedge(const Arg *arg) {
    ...
    if (!c || !arg) return;
    if (selmon->lt[selmon->sellt]->arrange && !c->isfloating) return;
    if (sscanf((char *) arg->v, "%c", &e) != 1) return;

    if (e == 't') ny = starty;

    if (e == 'b') ny = c->h > selmon->mh - 2 * c->bw ? c->h - bp : selmon->mh - c->h - 2 * c->bw - bp;

    if (e == 'l') nx = selmon->mx;

    if (e == 'r') nx = c->w > selmon->mw - 2 * c->bw ? selmon->mx + c->w : selmon->mx + selmon->mw - c->w - 2 * c->bw;
    ...
    if (e == 'B') nh = c->h + c->y + 2 * c->bw + bp == selmon->mh ? c->oldh : selmon->mh - c->y - 2 * c->bw - bp;
...
void opacity(Client *c, double opacity) {
    if (bUseOpacity && opacity > 0 && opacity < 1) {
        ...
        XChangeProperty(dpy, c->win, netatom[NetWMWindowsOpacity], XA_CARDINAL, 32, PropModeReplace,
                        (unsigned char *) real_opacity, 1);
    } else
...
void toggleforegrounded(const Arg *arg) {
    ...
    selmon->sel->isforegrounded || selmon->sel->isfloating ? detachforegrounded(selmon->sel)
                                                           : attachforegrounded(selmon->sel);

    selmon->sel->isforegrounded = selmon->sel->isfloating = !selmon->sel->isfloating && !selmon->sel->isforegrounded;
...
void col(Monitor *m) {
    ...
    for (i = x = y = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < m->nmaster) {
            ...
            resize(c, (m->rmaster ? x + m->wx + m->ww - mw : x + m->wx), m->wy, w - (2 * c->bw), m->wh - (2 * c->bw),
                   0);
            ...
            resize(c, (m->rmaster ? x + m->wx - m->ww : x + m->wx), m->wy + y, m->ww - x - (2 * c->bw), h - (2 * c->bw),
                   0);
...
void setmfact(const Arg *arg) {
    ...
    if (arg->f == 0.0) f = mfact;
    if (f < 0.05 || f > 0.95) return;
    selmon->mfact = f;
    for (i = 0; i < LENGTH(tags); ++i)
        if (selmon->tagset[selmon->seltags] & 1 << i) selmon->pertag->mfacts[i + 1] = f;
...
void setup(void) {
    ...
    for (j = LENGTH(colors) - 1; j >= 0; j--) {
        ...
        for (i = 0; i < SchemeN; i++) scheme[i] = drw_scm_create(drw, colors[j][i], alphas[i], 3);
    }
    /* init system tray */
    if (showsystray) updatesystray(0);
    /* init bars */
    updatebars();
    updatestatus();
...
void showhide(Client *c) {
    ...
    if (ISVISIBLE(c)) {
        ...
        if (isfakefullscreen) {
            if (!c->mon->lt[c->mon->sellt]->arrange || c->isfloating) resize(c, c->x, c->y, c->w, c->h, 0);
        } else {
    ...
    for (i = my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            ...
            resize(c, (m->rmaster ? m->wx + (ox * stairpx) : m->wx + mw + (ox * stairpx)), m->wy + (oy * stairpx),
                   m->ww - mw - (2 * c->bw) - (ow * stairpx), m->wh - (2 * c->bw) - (oh * stairpx), 0);
        }
    }
}
...
void togglebar(const Arg *arg) {
    ...
    for (i = 0; i < LENGTH(tags); ++i)
        if (selmon->tagset[selmon->seltags] & 1 << i) selmon->pertag->showbars[i + 1] = selmon->showbar;
...
void toggleall(const Arg *arg) {
    ...
    for (m = mons; m; m = m->next) {
        ...
        if ((arg->ui & TAGMASK) == m->tagset[m->seltags]) return;
        ...
        if (m->showbar != m->pertag->showbars[m->pertag->curtag]) togglebar(NULL);
...
void togglefloating(const Arg *arg) {
    ...
    if (selmon->sel->isfloating) {
        ...
        if (selmon->sel != 0) {
            ...
            resize(selmon->sel, selmon->sel->x, selmon->sel->y + user_bh, selmon->sel->w, selmon->sel->h, 0);
        }
    ...
    if (!selmon->sel->isfloating && selmon->sel->crop) cropdelete(selmon->sel);
    arrange(selmon);
}
...
void togglealwaysontop(const Arg *arg) {
    ...
    if (selmon->sel->isalwaysontop) {
        ...
        for (Monitor *m = mons; m; m = m->next)
            for (Client *c = m->clients; c; c = c->next) c->isalwaysontop = 0;
...
void togglermaster(const Arg *arg) {
    ...
    if (selmon->lt[selmon->sellt]->arrange) arrange(selmon);
}
...
Client *findbefore(Client *c) {
    Client *tmp;
    if (c == selmon->clients) return NULL;
...
void updatebars(void) {
    ...
    for (m = mons; m; m = m->next) {
        ...
        w = m->ww;
        m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by + vp, w - 2 * sp, bh, 0, depth, InputOutput, visual,
...
Monitor *wintomon(Window w) {
    ...
    if ((c = wintoclient(w)) || (c = cropwintoclient(w))) return c->mon;
    return selmon;
}
...
void resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst) {
    ...
    if (!(ret.addr == NULL || strncmp("String", type, 64))) {
        switch (rtype) {
        case STRING: strcpy(sdst, ret.addr); break;
        case INTEGER: *idst = strtoul(ret.addr, NULL, 10); break;
        case FLOAT: *fdst = strtof(ret.addr, NULL); break;
        }
    }
}
...
void previewallwin(const Arg *arg) {
    if (previewactive) { /* toggle off: open selected (or first) window and close preview */
        previewactive = 0;
        return;
    }
    ...
    for (n = 0, c = m->clients; c; c = c->next, n++) {
        if (c->isfullscreen) togglefullscr(&(Arg){0});
        /* If you hit awesomebar patch Unlock the notes below */
        // if (HIDDEN(c))
        //   continue;
        c->pre.orig_image = getwindowximage(c);
        if (!c->pre.orig_image) c->pre.orig_image = previewblankimage();
    }
    if (n == 0) return;
    ...
    for (c = m->clients; c; c = c->next) {
        if (!c->pre.win) {
            ...
            wa.border_pixel = WhitePixel(dpy, screen);
            c->pre.win = XCreateWindow(dpy, root, c->pre.x, c->pre.y, c->pre.scaled_image->width,
                                       c->pre.scaled_image->height, 1, depth, InputOutput, visual,
                                       CWColormap | CWOverrideRedirect | CWBackPixel | CWBorderPixel, &wa);
        } else
            XMoveResizeWindow(dpy, c->pre.win, c->pre.x, c->pre.y, c->pre.scaled_image->width,
        ...
        if (c->pre.win) {
            ...
            XPutImage(dpy, c->pre.win, drw->gc, c->pre.scaled_image, 0, 0, 0, 0, c->pre.scaled_image->width,
                      c->pre.scaled_image->height);
        }
    }
    previewsel = m->clients;
    previewactive = 1;
    previewhighlight();
    ...
    while (previewactive) {
        ...
        if (event.type == KeyPress) {
            ...
            unsigned int i;
            for (i = 0; i < LENGTH(keys); i++)
                if (keysym == keys[i].keysym && CLEANMASK(keys[i].mod) == CLEANMASK(event.xkey.state) &&
                    (keys[i].func == previewnext || keys[i].func == previewprev || keys[i].func == previewopen ||
                     keys[i].func == previewcancel || keys[i].func == previewallwin))
                    keys[i].func(&(keys[i].arg));
            continue;
        }
        ...
        if (event.type == LeaveNotify) previewhighlight();
    }
    XUngrabKeyboard(dpy, CurrentTime);
    previewactive = 0;
    focus_c = previewclose(m, previewsel);
    arrange(m);
    focus(focus_c);
}

void previewnext(const Arg *arg) {
    if (!previewactive) return;
    previewsel = previewsel->next ? previewsel->next : selmon->clients;
    previewhighlight();
}

void previewprev(const Arg *arg) {
    Client *it, *prev = NULL, *tail = NULL;
    if (!previewactive || !previewsel) return;
    for (it = selmon->clients; it; it = it->next) {
        if (it == previewsel) break;
        prev = it;
    }
    if (prev)
        previewsel = prev;
    else {
        for (tail = selmon->clients; tail->next; tail = tail->next);
        previewsel = tail;
    }
    previewhighlight();
}

void previewopen(const Arg *arg) {
    if (previewactive) previewactive = 0;
}

void previewcancel(const Arg *arg) {
    if (!previewactive) return;
    previewsel = NULL;
    previewactive = 0;
}

Client *previewclose(Monitor *m, Client *target) {
    Client *c;

    if (target) {
        selmon->seltags ^= 1; /* toggle sel tagset */
        m->tagset[selmon->seltags] = target->tags;
        focus(NULL);
    }
    for (c = m->clients; c; c = c->next) {
...
void previewhighlight() {
    Client *c;
    for (c = selmon->clients; c; c = c->next)
        XSetWindowBorder(dpy, c->pre.win,
                         (c == previewsel) ? scheme[SchemeSel][ColBorder].pixel : scheme[SchemeNorm][ColBorder].pixel);
}
...
void setpreviewwindowsizepositions(unsigned int n, Monitor *m, unsigned int gappo, unsigned int gappi) {
    ...
    cy = m->wy + (m->wh - cy) / 2;
    for (c = m->clients; c; c = c->next) c->pre.y += cy;
}
...
XImage *previewblankimage(void) {
    XImage *img = XCreateImage(dpy, visual, depth, ZPixmap, 0, NULL, 1, 1, 32, 0);
    if (!img) return NULL;
    img->data = calloc(1, img->bytes_per_line * img->height);
    return img;
}
...
XImage *scaledownimage(XImage *orig_image, unsigned int cw, unsigned int ch) {
    ...
    XImage *scaled_image = XCreateImage(dpy, visual, depth, ZPixmap, 0, NULL, scaled_width, scaled_height, 32, 0);
