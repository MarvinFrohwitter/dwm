...
void setgaps(int oh, int ov, int ih, int iv) {
  ...
  selmon->pertag->gaps[selmon->pertag->curtag] =
      ((oh & 0xFF) << 0) | ((ov & 0xFF) << 8) | ((ih & 0xFF) << 16) |
      ((iv & 0xFF) << 24);
  arrange(selmon);
}
...
static void bstack(Monitor *m) {
  ...
  if (m->nmaster && n > m->nmaster) {
    sh = (mh - ih) * (1 - (m->rmaster ? 1.0 - m->mfact : m->mfact));
    mh = mh - ih - sh;
    sx = mx;
    sy = my + mh + ih;
  }
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (i < m->nmaster) {
      resize(c, mx, (m->rmaster ? my + m->wh - mh - 2 * oh : my),
...
void centeredmaster(Monitor *m) {
  ...
  if (m->nmaster && n > m->nmaster) {
    ...
    if (n - m->nmaster > 1) {
      /* ||<-S->|<---M--->|<-S->|| */
      mw = (m->ww - 2 * ov - 2 * iv) * (m->rmaster ? 1.0 - m->mfact : m->mfact);
      lw = (m->ww - mw - 2 * ov - 2 * iv) / 2;
      rw = (m->ww - mw - 2 * ov - 2 * iv) - lw;
      mx += lw + iv;
    } else {
      /* ||<---M--->|<-S->|| */
      mw = (mw - iv) * (m->rmaster ? 1.0 - m->mfact : m->mfact);
      lw = 0;
      rw = m->ww - mw - iv - 2 * ov;
    }
    lx = m->wx + ov;
...
void centeredfloatingmaster(Monitor *m) {
  ...
  if (m->nmaster && n > m->nmaster) {
    ...
    /* go mfact box in the center if more than nmaster clients */
    if (m->ww > m->wh) {
      mw = m->ww * (m->rmaster ? m->mfact : 1.0 - m->mfact) -
           iv * mivf * (MIN(n, m->nmaster) - 1);
      mh = m->wh * 0.9;
    } else {
      mw = m->ww * 0.9 - iv * mivf * (MIN(n, m->nmaster) - 1);
      mh = m->wh * (m->rmaster ? m->mfact : 1.0 - m->mfact);
    }
    mx = m->wx + (m->ww - mw) / 2;
...
void deck(Monitor *m) {
  ...
  if (m->nmaster && n > m->nmaster) {
    sw = (mw - iv) * (1 - (m->rmaster ? 1.0 - m->mfact : m->mfact));
    mw = mw - iv - sw;
    sx = mx + mw + iv;
    sh = m->wh - 2 * oh;
  }
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      resize(c, (m->rmaster ? mx + m->ww - mw - 2 * ov : mx), my,
             mw - (2 * c->bw),
             (mh / mfacts) + (i < mrest ? 1 : 0) - (2 * c->bw), 0);
      my += HEIGHT(c) + ih;
    } else {
      resize(c, (m->rmaster ? mx : sx), sy, sw - (2 * c->bw), sh - (2 * c->bw),
             0);
    }
}
    ...
    if (r) {
      ...
      if (i == 0) {
        if (n != 1) {
          nw = (m->ww - iv - 2 * ov) -
               (m->ww - iv - 2 * ov) *
                   (1 - (m->rmaster ? 1.0 - m->mfact : m->mfact));
          wrest = 0;
        }
        ny = m->wy + oh;
      } else if (i == 1)
...
static void tile(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      ...
      resize(c, m->rmaster ? mx : sx, sy, sw - (2 * c->bw),
             (sh / sfacts) + ((i - m->nmaster) < srest ? 1 : 0) - (2 * c->bw),
             0);
