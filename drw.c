...
Drw *drw_create(Display *dpy, int screen, Window root, unsigned int w, unsigned int h, Visual *visual,
...
void drw_clr_create(Drw *drw, Clr *dest, const char *clrname, unsigned int alpha) {
    ...
    /* dest->pixel |= 0xff << 24; /1* This is the recomendation form the Arch
     * alacitty wiki *1/ */
...
void drw_scm_free(Drw *drw, Clr *scm, size_t clrcount) {
    ...
    for (i = 0; i < clrcount; i++) drw_clr_free(drw, &scm[i]);
    free(scm);
}
...
int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert) {
    ...
    while (1) {
        ...
        if (!*text || overflow) {
            ...
            if (nomatches[h0] == utf8codepoint || nomatches[h1] == utf8codepoint) goto no_match;
